...
def get_view_id(view_name):
    return c.execute("select rowid from views where name=?", (view_name.lower(),)).fetchone()[0]

def save(object_id, view, data):
    view_id = get_view_id(view)
    c.execute("insert into objects values (?, ?, ?, ?);", (object_id, int(time.time() * 1000), view_id, json.dumps(data)))
